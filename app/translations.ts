export const translations = {
  de: {
    // Navigation
    about: "Über uns",
    experience: "Erlebnis",
    testimonials: "Erfahrungen",
    register: "Anmelden",
    house: "Haus",
    trails: "Trails",

    // Hero section
    heroDate: "18.-22. August 2027",
    heroSubtitle1: "Das größte Trailrunning-Camp in den Alpen",
    heroSubtitle2: "4 Nächte, 3 Tage Laufen, 100% Community",
    heroSubtitle3: "Jedes Level, jedes Tempo",
    learnMore: "Mehr erfahren",
    registerNow: "Jetzt anmelden",

    // Key features
    epicTrails: "TRAILS",
    epicTrailsDesc: "Erlebe atemberaubende Trails für alle Laufniveaus",
    sunsetRave: "SUNSET RAVE",
    sunsetRaveDesc: "Tanz die müden Beine locker bei unserer legendären Summit-Party",
    community: "COMMUNITY",
    communityDesc: "Connecte dich mit Laufenden aus ganz Europa",

    // About section
    aboutTitle: "Das erwartet dich",
    aboutSubtitle:
      "Bei The Mountaincamp glauben wir, dass Laufen mehr ist als nur Kilometer schrubben – es geht darum, Grenzen zu überwinden, Gemeinschaft zu bilden und das Leben zu feiern.",
    aboutText1: "Nicht einfach ein Camp. Mehr als nur Laufen.\nTrails am Morgen, Abkühlung im See, echte Momente.",
    aboutText2: "Für alle Levels. Kein Leistungsdruck.\nFür alle, die mehr als nur Kilometer suchen.",
    aboutText3: "Laufen. Verbinden. Erleben.\nThe Mountaincamp.",
    // New bullet points for what's included
    aboutFeature1: "4 Nächte auf 1700m Höhe",
    aboutFeature2: "Frühstück, kleiner Lunch & Abendessen",
    aboutFeature3: "Tägliche Trailruns durch atemberaubende Natur",
    aboutFeature4: "Sauna & Recovery Lounge",
    aboutFeature5: "Aktivitäten | Workshops | Vorträge",
    aboutFeature6: "Snacks & Gels für deine Läufe",

    // Experience section
    experienceTitle: "Das Erlebnis",
    experienceSubtitle: "Alles inklusive in deinem unvergesslichen Abenteuer",

    // Feature cards
    trailrunningTitle: "Trailrunning & Abenteuer",
    trailrunningDesc: "Tägliche Trailruns auf atemberaubenden Alpenpfaden, konzipiert für alle Erfahrungsstufen.",
    recoveryTitle: "Erholung & Gemeinschaft",
    recoveryDesc: "Yoga, Eisbäder, Saunen und Geschichten am Lagerfeuer zum Aufladen und Verbinden.",
    creativityTitle: "Kreativität & Wachstum",
    creativityDesc:
      "Inspirierende Gespräche, aktive & kreative Erholung und eine Energie, die dich für das Leben begeistern wird.",
    raveTitle: "Sunset Rave",
    raveDesc: "Nach dem Laufens kannst du die müden Beine bei der Summit-Party ausschütteln",

    // Mountain Rave section
    raveHighlightTitle: "SUNSET RAVE",
    raveHighlightDesc:
      "Erlebe die ultimative Feier von Laufen, Musik und Gemeinschaft. Unser legendärer Sunset Rave findet auf einem Berggipfel statt, mit Panoramablick, unglaublicher Musik und einer Atmosphäre, die du nie vergessen wirst.",
    raveBullet1: "Live DJ-Sets mit Bergblick",
    raveBullet2: "Beine ausschütteln",
    raveBullet3: "Feiere mit deiner Community",

    // Activities section
    activitiesTitle: "ZUSÄTZLICH BUCHBARE AKTIVITÄTEN",
    activitiesSubtitle:
      "Von anspruchsvollen Gipfelläufen bis hin zu entspannenden See-Aktivitäten gibt es für jedes Energieniveau etwas",

    // Gallery section
    galleryTitle: "Camp-Momente",

    // Testimonials section
    testimonialsTitle: "Was Läufer sagen",
    playlistTitle: "DIE MOUNTAINCAMP PLAYLIST",
    playlistDesc:
      "Komm in Stimmung mit unserer offiziellen Trailrunning-Playlist. Der perfekte Soundtrack für deine Bergabenteuer.",
    // Registration section
    joinTitle: "Begleite uns in den Alpen",
    joinDate: "18.-22. August 2027",
    joinLocation: "Österreichische Alpen",
    accommodation: "4 Übernachtungen inklusive",
    meals: "Frühstück, kleiner Lunch & Abendessen inklusive",
    allLevels: "Geeignet für alle Laufniveaus",
    limitedSpots: "Begrenzte Plätze verfügbar",
    packageTitle: "KOMPLETTPAKET",
    perPerson: "pro Person",
    bookSpot: "Sei als Erste:r dabei",
    bookDesc: "Der Early-Bird-Verkauf ist vorbei. Trag dich ein und wir informieren dich über den nächsten Ticket-Release.",
    comingSoonBadge: "Anmeldung startet in Kürze",
    earlyBirdBadge: "Early Bird ist vorbei · Phase 2: Mittwoch, 26. August für 500€",
    notifyTitle: "Early Bird ist vorbei",
    notifyDesc: "Der Early-Bird-Verkauf ist beendet. Als Nächstes startet Phase 2 am Mittwoch, 26. August, von 00:00 bis 24:00 Uhr zum Preis von 500€. Trag deine E-Mail-Adresse ein und wir erinnern dich rechtzeitig.",
    notifyPlaceholder: "Deine E-Mail-Adresse",
    notifyButton: "Benachrichtigen",
    notifySubmitting: "Wird gesendet...",
    notifySuccess: "Danke! Wir melden uns, sobald die Tickets da sind.",
    notifyErrorInvalid: "Bitte gib eine gültige E-Mail-Adresse ein.",
    notifyErrorGeneric: "Etwas ist schiefgelaufen. Bitte versuche es erneut.",

    // Footer
    navigation: "Navigation",
    contact: "Kontakt",
    followUs: "Folge uns",
    legal: "Rechtliches",
    privacyPolicy: "Datenschutzerklärung",
    imprint: "Impressum",
    cookieSettings: "Cookie-Einstellungen",
    rights: "Alle Rechte vorbehalten",
    agb: "AGB",

    // Mobile
    tapForDetails: "Tippen für Details",
    tapToClose: "Tippen zum Schließen",

    // House page
    houseTitle: "HAUS",
    houseSubtitle: "Dein Zuhause in den Bergen",
    houseDescription: "Unser gemütliches Berghaus bietet dir allen Komfort, den du nach einem Tag auf den Trails brauchst.",
    facilitiesTitle: "AUSSTATTUNG",
    roomsTitle: "ZIMMER",
    amenitiesTitle: "ANNEHMLICHKEITEN",
    locationTitle: "LAGE",
    houseMainDescription: "Dieses gemütliche Gästehaus liegt auf 1.700 Metern Höhe und bietet einen atemberaubenden Blick auf die umliegenden Gipfel. Mit komfortablen Zimmern, einem großzügigen Gemeinschaftsbereich, einer Sauna und einer Turnhalle und vielem mehr ist es der perfekte Ort, um nach einem Tag auf den Trails zu entspannen.",
    houseMeals: "Morgens, Mittags und Abends werden wir vom Küchenteam mit reichhaltigen Mahlzeiten verpflegt.",
    cozySleepingAreas: "Gemütliche Schlafplätze",
    cozySleepingDesc: "Komfortable Schlafbereiche mit warmer Holzausstattung",
    sharedDormitory: "Mehrbettzimmer",
    sharedDormitoryDesc: "Gemütliche Mehrbettzimmer für 6-8 Personen mit Bergblick",
    bathroomFacilities: "BADEZIMMER & SANITÄRANLAGEN",
    washArea: "Waschbereich",
    washAreaDesc: "Moderne Waschbecken",
    showers: "Duschen",
    showersDesc: "Saubere Duschkabinen",
    toilets: "WC",
    toiletsDesc: "Moderne Sanitäranlagen",
    houseLocation: "Unser Berghaus befindet sich in Hochkrimml, Österreich, umgeben von atemberaubenden Alpengipfeln und endlosen Trailrunning-Möglichkeiten.",

    // Trails page
    trailsTitle: "TRAILS",
    trailsSubtitle: "Entdecke unsere atemberaubenden Laufstrecken",
    trailsDescription: "Von einfachen Pfaden bis zu anspruchsvollen Bergpfaden - hier findest du die perfekte Route für dein Niveau.",
    routesTitle: "UNSERE ROUTEN",
    difficultyTitle: "SCHWIERIGKEITSGRADE",
    dailyTrailRunsTitle: "TÄGLICHE TRAILRUNS",
    trailTipsTitle: "TRAIL-TIPPS",
    trailsMainDescription: "Die österreichischen Alpen bieten einige der spektakulärsten Trailrunning-Strecken Europas. Von sanften Waldwegen bis hin zu anspruchsvollen alpinen Pfaden - hier findet jeder Läufer die perfekte Route.",
    trailsGuideDescription: "Während des Mountaincamps bieten wir täglich Trailruns für verschiedene Niveaus an, sodass du die besten Trails der Region mit erfahrenen Guides erkunden kannst.",
    ourTrailRoutes: "Unsere Trailrouten",
    sampleRoutes: "Beispielrouten nach Schwierigkeit",
    dailyGuidedRuns: "Tägliche Trailruns",
    guidedRunsDescription: "Während des Mountaincamps bieten wir täglich mehrere Trailruns an, die auf verschiedene Erfahrungsstufen und Vorlieben zugeschnitten sind:",
    beginnerGroup: "Anfänger-Gruppe",
    beginnerGroupDesc: "Gemütliches Tempo, flachere Trails, perfekt für Trailrunning-Neulinge.",
    intermediateGroup: "Mittlere Gruppe",
    intermediateGroupDesc: "Moderates Tempo, abwechslungsreiche Trails mit mittleren Höhenunterschieden.",
    advancedGroup: "Fortgeschrittene Gruppe",
    advancedGroupDesc: "Schnelleres Tempo, technisch anspruchsvollere Trails mit größeren Höhenunterschieden.",
    equipment: "Ausrüstung",
    safety: "Sicherheit",
    trailShoes: "Trailrunning-Schuhe mit gutem Profil",
    hydrationPack: "Wasserflasche oder Hydration Pack",
    weatherJacket: "Wetterfeste Jacke",
    sunProtection: "Sonnenschutz",
    firstAidKit: "Kleine Erste-Hilfe-Ausrüstung",
    informSomeone: "Informiere immer jemanden über deine Route",
    checkWeather: "Überprüfe die Wettervorhersage",
    stayOnTrails: "Bleibe auf den markierten Wegen",
    respectNature: "Respektiere die Natur und Wildtiere",
    packOutTrash: "Nimm deinen Müll wieder mit",
    raffleConditions: "Teilnahmebedingungen Gewinnspiel",
    busTransport: "Transport",
    partnersTitle: "MARKEN, MIT DENEN WIR ZUSAMMENARBEITEN",
    partnersSubtitle: "Wir arbeiten mit großartigen Marken zusammen, die unsere Vision teilen",
  },
  en: {
    // Navigation
    about: "About",
    experience: "Experience",
    testimonials: "Testimonials",
    register: "Register",
    house: "House",
    trails: "Trails",

    // Hero section
    heroDate: "August 18-22, 2027",
    heroSubtitle1: "A trailrunning camp in the Austrian Alps",
    heroSubtitle2: "4 nights, 3 days of running, 100% community",
    heroSubtitle3: "Every level, every pace",
    learnMore: "Learn More",
    registerNow: "Register Now",

    // Key features
    epicTrails: "EPIC TRAILS",
    epicTrailsDesc: "Experience breathtaking alpine routes designed for all running levels",
    sunsetRave: "SUNSET RAVE",
    sunsetRaveDesc: "Dance under the stars at our legendary mountain-top party",
    community: "COMMUNITY",
    communityDesc: "Connect with like-minded runners in an unforgettable shared experience",

    // About section
    aboutTitle: "What awaits you",
    aboutSubtitle:
      "At The Mountaincamp we believe running is about more than just miles — it's about pushing boundaries, building community, and celebrating life.",
    aboutText1: "Not just a camp. More than just running.\nTrail runs in the morning, lake dips, real moments.",
    aboutText2: "For every level. No pressure.\nFor those chasing more than just miles.",
    aboutText3: "Run. Connect. Experience.\nThe Mountaincamp.",
    // New bullet points for what's included
    aboutFeature1: "4 nights at 1700m altitude",
    aboutFeature2: "Breakfast, small lunch & dinner",
    aboutFeature3: "Daily trail runs",
    aboutFeature4: "Sauna & recovery lounge",
    aboutFeature5: "Activities | workshops | talks",
    aboutFeature6: "Snacks & gels",

    // Experience section
    experienceTitle: "The Experience",
    experienceSubtitle: "Everything included in your unforgettable mountain adventure",

    // Feature cards
    trailrunningTitle: "Trailrunning & Adventure",
    trailrunningDesc: "Daily trail runs on breathtaking alpine trails, designed for all experience levels.",
    recoveryTitle: "Recovery & Community",
    recoveryDesc: "Yoga, ice baths, saunas, and fireside storytelling to recharge and connect.",
    creativityTitle: "Creativity & Growth",
    creativityDesc:
      "Inspirational talks, Active & Creative Recovery, and an energy that will leave you fired up for life.",
    raveTitle: "Sunset Rave",
    raveDesc:
      "After a day of running, let loose at our mountain-top party, with music, lights, and a vibe like no other.",

    // Mountain Rave section
    raveHighlightTitle: "MOUNTAIN-TOP SUNSET RAVE",
    raveHighlightDesc:
      "Experience the ultimate celebration of running, music, and community. Our legendary sunset rave takes place on a mountain summit, with panoramic views, incredible music, and an atmosphere you'll never forget.",
    raveBullet1: "Live DJ sets with mountain views",
    raveBullet2: "Shaking out them tired legs",
    raveBullet3: "Celebrate with your running community",

    // Activities section
    activitiesTitle: "ADDITIONALLY BOOKABLE ACTIVITIES",
    activitiesSubtitle:
      "From challenging summit runs to relaxing lake activities, there's something for every energy level",

    // Gallery section
    galleryTitle: "Camp Moments",

    // Testimonials section
    testimonialsTitle: "What Runners Say",
    playlistTitle: "THE MOUNTAINCAMP PLAYLIST",
    playlistDesc:
      "Get in the mood with our official trailrunning playlist. The perfect soundtrack for your mountain adventures.",
    // Registration section
    joinTitle: "Join Us in the Alps",
    joinDate: "August 18-22, 2027",
    joinLocation: "Austrian Alps",
    accommodation: "4 nights accommodation included",
    meals: "Breakfast, small Lunch & Dinner included",
    allLevels: "Suitable for all running levels",
    limitedSpots: "Limited spots available",
    packageTitle: "WHOLE PACKAGE",
    perPerson: "per person",
    bookSpot: "Be the First to Know",
    bookDesc: "The early-bird sale is over. Sign up and we'll notify you about the next ticket release.",
    comingSoonBadge: "Registration opens soon",
    earlyBirdBadge: "Early bird is over · Phase 2: Wednesday 26 August for €500",
    notifyTitle: "Early bird is over",
    notifyDesc: "The early-bird sale has ended. Next up is Phase 2 on Wednesday, 26 August, from 00:00 to 24:00 CEST for €500. Leave your email and we'll remind you in time.",
    notifyPlaceholder: "Your email address",
    notifyButton: "Notify me",
    notifySubmitting: "Submitting...",
    notifySuccess: "Thanks! We'll let you know as soon as tickets are live.",
    notifyErrorInvalid: "Please enter a valid email address.",
    notifyErrorGeneric: "Something went wrong. Please try again.",

    // Footer
    navigation: "Navigation",
    contact: "Contact",
    followUs: "Follow Us",
    legal: "Legal",
    privacyPolicy: "Privacy Policy",
    imprint: "Imprint",
    cookieSettings: "Cookie Settings",
    rights: "All rights reserved",
    agb: "Terms & Conditions",

    // Mobile
    tapForDetails: "Tap for details",
    tapToClose: "Tap to close",

    // House page
    houseTitle: "HOUSE",
    houseSubtitle: "Your home in the mountains",
    houseDescription: "Our cozy mountain house offers all the comfort you need after a day on the trails.",
    facilitiesTitle: "FACILITIES",
    roomsTitle: "ROOMS",
    amenitiesTitle: "AMENITIES",
    locationTitle: "LOCATION",
    houseMainDescription: "This cozy mountain house is located at an altitude of 1,700 meters and offers breathtaking views of the surrounding peaks. With comfortable rooms, a spacious common area, a sauna, and a gym, a skatepark and a lot more! It's the perfect place to relax after a day on the trails.",
    houseMeals: "In the morning, at noon and in the evening, we are getting served by the kitchen team with rich meals.",
    cozySleepingAreas: "Cozy Sleeping Areas",
    cozySleepingDesc: "Comfortable sleeping areas with warm wooden furnishing",
    sharedDormitory: "Shared Dormitory",
    sharedDormitoryDesc: "Cozy shared rooms for 6-8 people with mountain views",
    bathroomFacilities: "BATHROOM & FACILITIES",
    washArea: "Wash Area",
    washAreaDesc: "Modern wash basins",
    showers: "Showers",
    showersDesc: "Clean shower facilities",
    toilets: "Toilets",
    toiletsDesc: "Modern toilet facilities",
    houseLocation: "Our mountain house is located in Hochkrimml, Austria, surrounded by breathtaking alpine peaks and endless trail running possibilities.",

    // Trails page
    trailsTitle: "TRAILS",
    trailsSubtitle: "Discover our breathtaking running routes",
    trailsDescription: "From easy paths to challenging mountain trails - find the perfect route for your level.",
    routesTitle: "OUR ROUTES",
    difficultyTitle: "DIFFICULTY LEVELS",
    dailyTrailRunsTitle: "DAILY TRAIL RUNS",
    trailTipsTitle: "TRAIL TIPS",
    trailsMainDescription: "The Austrian Alps offer some of the most spectacular trail running routes in Europe. From gentle forest paths to challenging alpine trails - every runner will find the perfect route here.",
    trailsGuideDescription: "During the Mountaincamp, we offer daily trail runs for different levels, allowing you to explore the best trails in the region with experienced guides.",
    ourTrailRoutes: "Our Trail Routes",
    sampleRoutes: "Sample Routes by Difficulty",
    dailyGuidedRuns: "Daily Trail Runs",
    guidedRunsDescription: "During the Mountaincamp, we offer multiple daily trail runs tailored to different experience levels and preferences:",
    beginnerGroup: "Beginner Group",
    beginnerGroupDesc: "Easy pace, flatter trails, perfect for trail running beginners.",
    intermediateGroup: "Intermediate Group",
    intermediateGroupDesc: "Moderate pace, varied trails with medium elevation gains.",
    advancedGroup: "Advanced Group",
    advancedGroupDesc: "Faster pace, more technically challenging trails with significant elevation gains.",
    equipment: "Equipment",
    safety: "Safety",
    trailShoes: "Trail running shoes with good traction",
    hydrationPack: "Water bottle or hydration pack",
    weatherJacket: "Weather-appropriate jacket",
    sunProtection: "Sun protection",
    firstAidKit: "Small first aid kit",
    informSomeone: "Always inform someone about your route",
    checkWeather: "Check the weather forecast",
    stayOnTrails: "Stay on marked trails",
    respectNature: "Respect nature and wildlife",
    packOutTrash: "Pack out what you pack in",
    raffleConditions: "Raffle Conditions",
    busTransport: "Transport",
    partnersTitle: "BRANDS WE'VE PARTNERED UP WITH",
    partnersSubtitle: "We work with amazing brands that share our vision",
  },
}

export type Language = "de" | "en"
export type TranslationKey = keyof typeof translations.en
